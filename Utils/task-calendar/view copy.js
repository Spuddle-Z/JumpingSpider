...
function getMeta(tasks) {
    ...
    for (i = 0; i < tasks.length; i++) {
		...
		if (priorityMatch) {
			...
			tasks[i].text = tasks[i].text.replace(priorityMatch[0], "");
		}
        ...
        var completionMatch = taskText.match(/\[( |x)\] /);
        if (completionMatch) {
			if (completionMatch[1] == " ") { tasks[i].completion = "F"; }
			else if (completionMatch[1] == "x") { tasks[i].completion = "T"; }
		...
		if (textMatch) {
			...
			tasks[i].text = tasks[i].text.replace(textMatch[0], "");
		}
    }
};
...
function getTasks(date) {
	tasks_today = tasks.filter(t => moment(t.due).isSame(date));
	done = tasks_today.filter(t => t.completion == "T");
	due = tasks_today.filter(t => t.completion == "F");
