...
- **纯策略纳什均衡(Pure Strategy Nash Equilibrium, PNE)**：字面意思，某个博弈的NE恰好是纯策略。*每个博弈都有NE，但其不一定是PNE。* ^k3jr2l
...
> 对于一个博弈，如果存在一个**势函数(Potential Function)**$\Phi$，能够反映每个玩家的效用函数，就称这个博弈为**位势博弈**。
...
- **Flow Routing Game**：其研究内容是对于一个有向图的路径选择问题。其符号表示如下：
...
\end{aligned}
$$
- **Wardrop均衡(Wardrop Equilibrium, WA)**：Flow routing博弈下的均衡状态一定满足以下条件
...
此处我们需要证明拥塞博弈存在一个[[博弈论#^k3jr2l|PNE]]。由此，我们构造一个势函数
$$\Phi(a)=\sum_{j=1}^{m}\sum_{k=1}^{\eta_j(a)}C_j(k)$$
仔细观察这个势函数，其本质就是将刚刚flow routing博弈中凑互补松弛条件时构造的目标函数里面的积分，换成了求和。

此时需要证明
$$\Phi(a_i,a_{-i})-\Phi(a_i',a_{-i})=u_i(a_i,a_{-i})-u_i(a_i,a_{-i})$$
这使得最小化这个势函数$\Phi$，就等价于最小化每一个玩家的代价$u$。以下为证明过程：
> [!proof] 
