...
- **角谷静夫不动点定理(Kakutani Fixed Point Theorem)**：若集合$C\subseteq\mathbb{R}$为有界闭凸集，$f:C\to C$是一个具有[[Discrete Mathematics 离散数学#^x840vj|闭图]]的映射，且对于$\forall x\in C$，$f(x)$是一个非空凸集，则$f$存在一个不动点。
...
其中$a,b\in\Omega$，那么若$\forall b$都有$L(a,a)\geq L(b,a)$，则$a$是一个NE。